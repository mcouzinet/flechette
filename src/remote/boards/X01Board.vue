  ...
  <div class="flex flex-col gap-3 xl:gap-5">
    ...
    <div class="rounded-[14px] p-3 xl:p-5" style="border: 2px dashed var(--chalk-line)">
      ...
      <div class="grid grid-cols-2 gap-2 mb-3 xl:flex xl:flex-wrap xl:gap-2 xl:mb-5 xl:justify-center">
        <button v-for="type in ['single','double','triple']" :key="type" @click="scoreType = type" class="x01-type-btn"
          :style="typeStyle(type, 'var(--chalk-gold)')">{{ typeLabel(type) }}</button>
        <button @click="selectMiss" class="x01-type-btn" :style="typeStyle('miss', 'var(--chalk-red)')">Manqué</button>
      </div>

      <div class="grid grid-cols-4 lg:grid-cols-7 gap-1.5 mb-3 xl:gap-2 xl:mb-5">
        <button v-for="n in dartNumbers" :key="n" @click="selectScore(n)" class="x01-num-btn"
...
.x01-type-btn { font-family: var(--font-display); letter-spacing: 0.5px; font-size: 22px; border: 2px solid; border-radius: 14px; padding: 16px 0; cursor: pointer; line-height: 1.1; transition: all 0.2s; }
.x01-type-btn:hover { background: rgba(241,230,203,0.06); }
.x01-num-btn { height: 44px; border: 2px solid; border-radius: 10px; font-family: var(--font-display); font-size: 17px; letter-spacing: 0.5px; cursor: pointer; transition: all 0.15s; background: transparent; }
.x01-num-btn:hover { background: rgba(241,230,203,0.06); border-color: var(--chalk-gold); }
@media (min-width: 1280px) {
  .x01-type-btn { padding: 16px 24px; }
  .x01-num-btn { height: 54px; font-size: 20px; }
}
</style>
